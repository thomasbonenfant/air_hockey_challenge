...
class AirHockeyPlanarAtacom(AtacomEnvWrapper):
    def __init__(self, env_info, mdp_info, policy_class, policy_params, actor_params, actor_optimizer,
                 critic_params, batch_size, initial_replay_size, max_replay_size, tau, task='H',
                 timestep=1 / 240., Kc=240., **kwargs):
        if task == 'H':
            agent = HittingAgent(env_info, mdp_info, policy_class, policy_params, actor_params, actor_optimizer,
        ...
        vel_max = env_info['constraints'].get('joint_vel_constr')
        super().__init__(base_env, 3, f=None, g=g, Kc=Kc, vel_max=vel_max, acc_max=acc_max, Kq=2 * acc_max / vel_max,
                         time_step=timestep)

        self.robot_model = env_info['robot']['robot_model']
        self.robot_data = env_info['robot']['robot_data']
        self.frame_idx = self.robot_model.nframes - 1

        """
        NOT SURE ABOUT THE FOLLOWING LINES, may refer to pybullet
        self.env.client.changeDynamics(*base_env._indexer.joint_map[self.robot_model.names[1]],
                                       maxJointVelocity=self.robot_model.velocityLimit[0] * 1.5)
        self.env.client.changeDynamics(*base_env._indexer.joint_map[self.robot_model.names[2]],
                                       maxJointVelocity=self.robot_model.velocityLimit[1] * 1.5)
        self.env.client.changeDynamics(*base_env._indexer.joint_map[self.robot_model.names[3]],
                                       maxJointVelocity=self.robot_model.velocityLimit[2] * 1.5)
        """
    ...
    def cart_pos_J_g(self, q):
        ...
        """
        #env_info['constraints'].get('ee_constr').jacobian(q, dq)
        ee_jac = pino.computeFrameJacobian(self.robot_model, self.robot_data, q,
                                           self.frame_idx, pino.LOCAL_WORLD_ALIGNED)[:2]
        J_c = np.array([[-1., 0.], [0., -1.], [0., 1.]])
        return J_c @ ee_jac

    def cart_pos_b_g(self, q, dq):
        pino.forwardKinematics(self.robot_model, self.robot_data, q, dq)
        acc = pino.getFrameClassicalAcceleration(self.robot_model, self.robot_data, self.robot_model.nframes - 1,
                                                 pino.LOCAL_WORLD_ALIGNED).vector
        J_c = np.array([[-1., 0.], [0., -1.], [0., 1.]])
        return J_c @ acc[:2]

    def joint_pos_g(self, q):
        return np.array(q ** 2 - self.robot_model.upperPositionLimit ** 2)
    ...
    def joint_vel_g(self, q, dq):
        return np.array([dq ** 2 - self.robot_model.velocityLimit ** 2])
        ...
        for data in dataset:
            ...
            if data[-1]:
                ...
                for state_i in state_hist:
                    pino.framesForwardKinematics(self.robot_model, self.robot_data, state_i[6:9])
                    ee_pos_list.append(self.robot_data.oMf[-1].translation[:2] + self.env.agents[0]['frame'][:2, 3])
                ...
                axes2[0].plot([0, state_hist.shape[0]], [self.robot_model.lowerPositionLimit[0]] * 2,
                              label='position limit', c='tab:red', ls='--')
                axes2[1].plot([0, state_hist.shape[0]], [self.robot_model.lowerPositionLimit[1]] * 2, c='tab:red',
                              ls='--')
                axes2[2].plot([0, state_hist.shape[0]], [self.robot_model.lowerPositionLimit[2]] * 2, c='tab:red',
                              ls='--')
                axes2[0].plot([0, state_hist.shape[0]], [self.robot_model.upperPositionLimit[0]] * 2, c='tab:red',
                              ls='--')
                axes2[1].plot([0, state_hist.shape[0]], [self.robot_model.upperPositionLimit[1]] * 2, c='tab:red',
                              ls='--')
                axes2[2].plot([0, state_hist.shape[0]], [self.robot_model.upperPositionLimit[2]] * 2, c='tab:red',
                              ls='--')
                ...
                axes2[0].plot([0, state_hist.shape[0]], [-self.robot_model.velocityLimit[0]] * 2,
                              label='velocity limit', c='tab:pink', ls=':')
                axes2[1].plot([0, state_hist.shape[0]], [-self.robot_model.velocityLimit[1]] * 2, c='tab:pink', ls=':')
                axes2[2].plot([0, state_hist.shape[0]], [-self.robot_model.velocityLimit[2]] * 2, c='tab:pink', ls=':')
                axes2[0].plot([0, state_hist.shape[0]], [self.robot_model.velocityLimit[0]] * 2, c='tab:pink', ls=':')
                axes2[1].plot([0, state_hist.shape[0]], [self.robot_model.velocityLimit[1]] * 2, c='tab:pink', ls=':')
                axes2[2].plot([0, state_hist.shape[0]], [self.robot_model.velocityLimit[2]] * 2, c='tab:pink', ls=':')
