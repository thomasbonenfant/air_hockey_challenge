...
from utils.ATACOM_transformation import AtacomTransformation
...
from utils.ATACOM_agent import ATACOMAgent
...
class Agent(AgentBase):
    def __init__(self, env_info, varient, **kwargs):
        ...
        if self.use_atacom:
            ...
            if self.dim == 3:
                ...
                self.atacom_transformation = ATACOMAgent(env_info, self.get_joint_pos, self.get_joint_vel)
                low_action = low_action[:7 - self.atacom_transformation.n_equality_constraints]
                high_action = high_action[:7 - self.atacom_transformation.n_equality_constraints]
    ...
    def load_agent(self, path, baseline_agent=False, env_label=None, random_agent=False):
    ...
    def draw_action(self, observation):
        ...
        if self.use_atacom:
            action = self.atacom_transformation.draw_action(scaled_action, self.joint_pos, self.joint_vel)
            self.clipped_state = False
        else:
...
class HitAgent(Agent):
    def __init__(self, env_info, **kwargs):
        ...
        env_label = "7dof-hit"
        trainer, self.env12 = self.load_agent(path, baseline_agent=False, env_label=env_label, random_agent=False)
