...
from MagicRL.algorithms.utils import LearnRates, check_directory_and_create, ParamSamplerResults
from MagicRL.data_processors import IdentityDataProcessor
from MagicRL.algorithms.samplers import *
...
class PGPE:
    ...
    def __init__(
        ...
        Args:
            ...
            env_maker (Callable, optional): The environment's constructor
            
            policy_maker (Callable, optional): The parametric policy's constructor
        ...
        assert env_maker is not None, "[ERROR] No env constructor provided."
        self.env_maker = env_maker
        self.env_args = env_args

        assert policy_maker is not None, "[ERROR] No policy constructor provided."
        ...
        self.sampler = ParameterSampler(
            env_maker=self.env_maker, env_args=self.env_args, policy_maker=self.policy_maker,
            policy_args=self.policy_args, data_processor=self.data_processor,
    ...
    def collect_trajectory(self, params: np.array, starting_state=None) -> float:
        ...
        env = self.env_maker(**self.env_args)
        policy = self.policy_maker(**self.policy_args)
        policy.reset()

        env.reset()
        if starting_state is not None:
            env.state = copy.deepcopy(starting_state)

        # initialize parameters
        perf = 0
        policy.set_parameters(thetas=params)
        ...
        for t in range(env.horizon):
            ...
            # select the action
            a = policy.draw_action(state=features)

            # play the action
            _, rew, abs = env.step(action=a)
