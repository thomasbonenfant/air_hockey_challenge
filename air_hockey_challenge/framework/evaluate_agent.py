...
def _evaluate(log_dir, env, agent_builder, init_states, quiet, render, episode_offset, seed, i, interpolation_order, **kwargs):
    ...
    mdp = CustomEnvironmentWrapper(env,interpolation_order=interpolation_order)
    ...
    core = ChallengeCore(agent, mdp, is_tournament=False, init_state=mdp.base_env.init_state, action_idx=[i for i in range(3)])
