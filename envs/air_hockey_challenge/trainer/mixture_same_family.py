...
class MixtureSameFamily(Distribution):
    ...
    def __init__(self,
        ...
        #print(component_distribution.event_shape)
    ...
    def sample(self, sample_shape=torch.Size()):
        with torch.no_grad():
            ...
            #print(mix_shape + torch.Size([1] * (len(es) + 1)))
            '''
            ...
            '''
            #mix_sample_r = mix_sample.reshape([comp_samples.shape[0], comp_samples.shape[1], 1])
            mix_sample_r = mix_sample.reshape([comp_samples.shape[0], 1])

            #print(comp_samples, mix_sample_r, gather_dim)

            samples = torch.gather(comp_samples, gather_dim, mix_sample_r)
            return samples #samples.squeeze(gather_dim)
